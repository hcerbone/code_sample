...
void m61_free(void* ptr, const char* file, long line) {
    ...
    if(ptr){
        if(!(heap_min_track <= ptr_addr && ptr_addr <= heap_max_track)) {
            ...
            if(header->status_tag != magic_header && header->status_tag != magic_free){
                fprintf(stderr, "MEMORY BUG: %s:%lu: invalid free of pointer %p, not allocated", file, line, ptr);
                abort();
            } else if(memcmp(footer_ptr, &magic_footer, magic_footer_sz) != 0){
...
void m61_print_leak_report() {
    ...
    while(curr_pointer != nullptr && curr_pointer != inter_ptr)
    {
        ...
        curr_pointer->alloc_file, curr_pointer->alloc_line, (char*)curr_pointer + meta_data_sz, curr_pointer->alloc_size);
