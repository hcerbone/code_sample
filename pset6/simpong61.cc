...
// mutexes for sticky places and holes
std::mutex mutex_sticky;
std::condition_variable_any cv_sticky;

std::mutex mutex_hole;
std::condition_variable_any cv_hole;
...
void ball_thread() {
  ...
  while (true) {
    ...
    if (mval > 0) {
      mutex_sticky.lock();
      while (ball->check_cell_stick()) {
        cv_sticky.wait(mutex_sticky);
      }
...
void summary_handler(int) {
  ...
  if (main_board) {
    ...
    pr << nstarted << " threads started, " << nrunning << " running, "
...
void signal_handler(int) {
  ...
  if (main_board) {
    ...
    for (int y = 0; y < main_board->height_; ++y) {
      ...
      nw = write(STDOUT_FILENO, buf, std::min(main_board->width_ + 1, BUFSIZ));
    }
...
int main(int argc, char **argv) {
  ...
  int width = 100, height = 31, nballs = 24, nsticky = 12, nholes = 0,
      nthreads = -1;
  ...
  if (optind != argc || width < 2 || height < 2 ||
      std::min(nthreads, nballs) + nsticky + nholes >= width * height ||
      nthreads == 0 || nthreads > nballs) {
    usage();
  }
  if (print_interval > 0) {
    struct itimerval it;
