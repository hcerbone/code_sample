...
enum pong_celltype { cell_empty, cell_sticky, cell_obstacle, cell_hole };
...
struct pong_ball {
  ...
  pong_ball(pong_board &board) : board_(board) {}
  ...
  void place() {
    ...
    while (!this->placed_) {
      ...
      this->board_.mutexes_per_row[y + 1].lock();
      ...
      this->board_.mutexes_per_row[y + 1].unlock();
    }
  }
  ...
  int move() {
    // build mutexes and locks for each row to allow the balls to move properly
    std::mutex *mutexes_per_row = this->board_.mutexes_per_row;
    std::lock(mutexes_per_row[this->y_], mutexes_per_row[this->y_ + 1],
              mutexes_per_row[this->y_ + 2]);
    std::unique_lock<std::mutex> oneGuard(mutexes_per_row[this->y_],
                                          std::adopt_lock);
    std::unique_lock<std::mutex> twoGuard(mutexes_per_row[this->y_ + 1],
                                          std::adopt_lock);
    std::unique_lock<std::mutex> threeGuard(mutexes_per_row[this->y_ + 2],
                                            std::adopt_lock);

    // return -1 if ball has been removed from board
    if (this->x_ < 0 || this->y_ < 0) {
      assert(this->x_ < 0 && this->y_ < 0 && this->dx_ == 0 && this->dy_ == 0);
      return -1;
    }
    ...
    // check next cell
    pong_cell &next_cell =
        board.cell(this->x_ + this->dx_, this->y_ + this->dy_);
    if (next_cell.ball_) {
  ...
  bool check_cell_stick() {
    std::unique_lock<std::mutex> guard(
        this->board_.mutexes_per_row[this->y_ + 1]);
    return this->dx_ == 0 && this->dy_ == 0;
  }
};
